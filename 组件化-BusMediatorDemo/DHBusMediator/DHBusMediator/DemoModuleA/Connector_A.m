...
{
    @autoreleasepool {
        [DHBusMediator registerConnector:[self sharedConnector]];
    }
}
...
#pragma mark - DHBusConnectProtocal
...
- (nullable UIViewController *)connectToOpenURL:(nonnull NSURL *)URL params:(nullable NSDictionary *)params{
    ...
    if ([URL.host isEqualToString:@"ADetail"]) {
        DHModuleAViewController *viewController = [[DHModuleAViewController alloc] init];
...
- (nullable id)connectToHandleProtocol:(nonnull Protocol *)servicePrt {
    if (servicePrt == @protocol(DemoModuleAServiceProtocal)) {
...
-(nonnull id<DemoModuleAItemProtocal>)moduleA_getItemWithName:(nonnull NSString *)name
                                                    age:(int)age{
    DemoModuleAItem *item = [[DemoModuleAItem alloc] initWithItemName:name itemAge:age];
    return item;
}
